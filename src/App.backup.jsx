...
console.error('🚀 [App.jsx] Evaluando importaciones...');

export default function App() {
  console.error('🚀 [App.jsx] Componente App() inició render ==============================');
  ...
  // ── EMERGENCY DEADLINE: If app hangs for 3s, force show login ──────────────
  const [forceReady, setForceReady] = useState(false);
  useEffect(() => {
    if (isAppReady) return; // Already ready, no timer needed
    const t = setTimeout(() => {
      console.warn("⚡ App deadline reached (3s). Forcing render.");
      // Clear any stale session that keeps us stuck
  ...
  // If force-ready triggered (3s timeout) OR normal condition: show login
  if (forceReady || (!session && !db.usuario)) {
    return <Login />;
  }

  // 3. Bloqueo de carga: asegurar que todos los datos iniciales estén listos
  if (!isAppReady && !forceReady) {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", width: "100vw", alignItems: "center", justifyContent: "center", background: T.bg0, color: T.white }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", color: T.whiteDim, fontSize: 13, fontWeight: 500 }}>
          <div style={{ width: 14, height: 14, border: `2px solid ${T.tealSoft}`, borderTopColor: T.teal, borderRadius: "50%", animation: "spin 1s linear infinite" }} />
