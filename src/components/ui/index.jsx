...
// ð¡ï¸ COMPONENTE ANTI-RERENDER: El arma definitiva contra la pérdida de texto.
...
export const Sel = ({ value, onChange, children, style = {}, innerStyle = {}, placeholder = "Seleccionar...", multiple = false }) => {
  ...
  useEffect(() => {
    const clickAfuera = (e) => { 
      if (ref.current && !ref.current.contains(e.target)) {
        const portal = document.getElementById("sel-portal-" + (ref.current?.id || ""));
    ...
    const handleScroll = (e) => {
      if (!open) return;
      // No cerrar si el scroll ocurre dentro del propio dropdown portal
      const portalId = "sel-portal-" + (ref.current?.id || "");
  ...
  const isSelected = (val) => {
    if (multiple) {
      return Array.isArray(value) ? value.includes(val) : (value === val && val !== "");
    }
    return value === val;
  };
  ...
  const currentLabel = () => {
    if (multiple) {
      const selected = options.filter(o => isSelected(o.val) && o.val !== "");
      if (selected.length === 0) return placeholder;
      if (selected.length === 1) return selected[0].lab;
      return `${selected.length} seleccionados`;
    }
    const found = options.find(o => o.val === value);
    return found ? found.lab : placeholder;
  };
  ...
  return (
    <div ref={ref} id={"sel-" + (value || "").toString().slice(0,8)} style={{ position: "relative", width: "100%", ...style }}>
      <div onClick={handleToggle}
        style={{ 
          background: "rgba(255,255,255,0.03)", 
          border: `1.5px solid ${T.whiteFade}`, 
          borderRadius: 14, 
          padding: "0 18px", 
          display: "flex", 
          ...
          height: style.height || innerStyle.height || 48,
          minHeight: 40,
          ...innerStyle,
          border: innerStyle.border || (open ? `1.5px solid ${T.teal}` : `1.5px solid ${T.whiteFade}`)
        }}
        onMouseEnter={e => {
          if (!open) e.currentTarget.style.borderColor = T.tealSoft;
          e.currentTarget.style.background = "rgba(255,255,255,0.06)";
        }}
        onMouseLeave={e => {
          if (!open) e.currentTarget.style.borderColor = innerStyle.border ? innerStyle.border : T.whiteFade;
          e.currentTarget.style.background = "rgba(255,255,255,0.03)";
        }}
      >
        <span style={{ fontSize: 13, color: txtColor, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, marginRight: 8, letterSpacing: "-0.01em" }}>
          {currentLabel()}
        </span>
        <Ico k="chevron-down" size={12} style={{ 
          transform: open ? (openUp ? "rotate(0deg)" : "rotate(180deg)") : (openUp ? "rotate(180deg)" : "none"), 
          transition: "all 0.4s cubic-bezier(0.19, 1, 0.22, 1)", 
          color: open ? T.teal : T.whiteFade, 
          flexShrink: 0 
        }} />
      </div>
      {open && rect && createPortal(
        <div 
          id={"sel-portal-" + (ref.current?.id || "")}
          style={{ 
            position: "fixed", 
            zIndex: 100000, 
            left: rect.left, 
            width: rect.width,
            [openUp ? "bottom" : "top"]: openUp ? (window.innerHeight - rect.top + 8) : (rect.bottom + 8),
            background: T.bg1, 
            ...
            maxHeight: 280, 
            overflowY: "auto", 
            animation: openUp ? "fadeInUp .3s ease-out" : "slideIn .3s ease-out",
            backdropFilter: "blur(30px)"
          }}>
          {options.map(o => (
            <div key={o.val} onClick={() => handleSelect(o.val)}
              style={{ 
                padding: "12px 16px", 
                borderRadius: 12, 
                fontSize: 13, 
                color: isSelected(o.val) ? T.teal : T.whiteOff, 
                background: isSelected(o.val) ? "rgba(20, 184, 166, 0.1)" : "transparent", 
                cursor: "pointer", 
                transition: "all 0.2s", 
                fontWeight: isSelected(o.val) ? 900 : 500, 
                display: "flex", 
                alignItems: "center", 
                justifyContent: "space-between",
                marginBottom: 2
              ...
              onMouseLeave={e => { e.currentTarget.style.background = isSelected(o.val) ? "rgba(20, 184, 166, 0.1)" : "transparent"; }}
            >
              <span>{o.lab}</span>
              {isSelected(o.val) && <Ico k="check" size={14} style={{ color: T.teal }} />}
            </div>
          ))}
...
export const Modal = ({ open, onClose, title, children, width = 640 }) => {
  if (open) console.log("ðŸ”˜ [DEBUG] Abriendo Modal:", title);
  if (!open) return null;
  return createPortal(
...
export const KPI = ({ label, value, sub, color = T.teal, icon, style = {} }) => (
  <Tarjeta style={{ padding: "18px 16px", flex: 1, minWidth: 180, ...style }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 1, gap: 8 }}>
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: T.whiteDim, textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 8, lineHeight: 1.2 }}>{label}</div>
        <div style={{ fontSize: "clamp(18px, 2vw, 24px)", fontWeight: 900, color: T.white, lineHeight: 1.1, marginBottom: 6, letterSpacing: "-.02em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={value}>{value}</div>
        {sub && <div style={{ fontSize: 12, color: color, fontWeight: 700, lineHeight: 1.2 }} title={sub}>{sub}</div>}
      </div>
      {icon && (
        <div style={{ position: "relative", flexShrink: 0 }}>
          {/* Aura / Glow */}
          <div style={{ position: "absolute", inset: -6, background: color, borderRadius: 10, opacity: 0.15, filter: "blur(10px)" }} />
          <div style={{ width: 36, height: 36, borderRadius: 10, background: `linear-gradient(135deg, ${color}22, ${color}11)`, border: `1px solid ${color}33`, color, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1 }}><Ico k={icon} size={18} /></div>
        </div>
      )}
    </div>
  </Tarjeta>
