          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            {(editRule?.tipo.includes("enviar") || editRule?.tipo === "notif_user" || editRule?.tipo === "send_chat_msg" || editRule?.tipo === "add_comment" || editRule?.tipo.includes("msg") || editRule?.tipo.includes("notif")) && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="mail" size={14} /> Contenido Automatizado</div>
                {editRule.tipo.includes("email") && (
                  <Campo label="Asunto del Correo">
                ...
                <Campo label="Cuerpo del Mensaje (Texto Plano)">
                  <textarea
                    value={editRule.config?.mensaje}
                    onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, mensaje: e.target.value } })}
                    style={{ width: "100%", height: 100, borderRadius: 12, border: `1px solid ${T.border}`, background: T.bg1, color: T.white, padding: 16, fontSize: 14, outline: "none", fontFamily: "inherit", resize: "none", lineHeight: "1.5" }}
                    placeholder="Escribe el mensaje... {nombre} se reemplazará automáticamente."
                  />
                </Campo>

                {editRule.tipo.includes("email") && (
                  <Campo label="Cuerpo HTML (Opcional)">
                    <textarea
                      value={editRule.config?.html}
                      onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, html: e.target.value } })}
                      style={{ width: "100%", height: 150, borderRadius: 12, border: `1px solid ${T.border}`, background: T.bg1, color: T.white, padding: 16, fontSize: 13, outline: "none", fontFamily: "monospace", resize: "none", lineHeight: "1.4" }}
                ...
                {editRule.tipo.includes("email") && (
                  <ThemedSelect
                    label="Enviar desde la cuenta:"
                    value={editRule.config?.account_id || ""}
                    options={(db.email_accounts || []).filter(acc => acc.active).map(acc => ({ id: acc.id, label: acc.email }))}
                    onChange={v => setEditRule({ ...editRule, config: { ...editRule.config, account_id: v } })}
                  />
                )}

                {(editRule.tipo === "enviar_wa" || editRule.tipo.includes("wa")) && (
                  <ThemedSelect
                    label="Enviar desde el canal de WhatsApp:"
                    value={editRule.config?.account_id || ""}
                    options={(db.whatsapp_accounts || []).map(acc => ({ id: acc.id, label: acc.nombre }))}
                    onChange={v => setEditRule({ ...editRule, config: { ...editRule.config, account_id: v } })}
                  />
                )}
              </div>
            ...
            {/* 2. CAMBIO DE ETAPA (PIPELINE) */}
            {editRule?.tipo === "change_stage" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="arrow" size={14} /> Movimiento en el Pipeline</div>
                <ThemedSelect
                  label="Mover a la etapa:"
                ...
                />
                <div style={{ fontSize: 12, color: T.whiteFade }}>Al cumplirse las condiciones, el negocio se moverá automáticamente a esta columna.</div>
              </div>
            )}
            ...
            {editRule?.tipo === "change_resp" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="user" size={14} /> Reasignación de Responsable</div>
                <ThemedSelect
                  label="Asignar a:"
                  value={editRule.config?.responsable_id || ""}
                  options={usuarios.map(u => ({ id: u.id, label: u.name }))}
            ...
            {editRule?.tipo === "create_task" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="check-circle" size={14} /> Generación de Tareas</div>
                <Campo label="Título de la tarea">
                  <Inp value={editRule.config?.titulo_tarea} onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, titulo_tarea: e.target.value } })} placeholder="Ej: Llamar para seguimiento comercial..." />
                </Campo>
                <Campo label="Descripción del pendiente">
                  <textarea
                    value={editRule.config?.desc_tarea}
                    onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, desc_tarea: e.target.value } })}
                    style={{ width: "100%", height: 80, borderRadius: 10, border: `1px solid ${T.border}`, background: T.bg1, color: T.white, padding: 12, fontSize: 13, outline: "none", fontFamily: "inherit", resize: "none" }}
            ...
            {(editRule?.tipo === "mod_item" || editRule?.tipo === "upd_contact" || editRule?.tipo === "upd_company") && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="edit" size={14} /> Actualización de Propiedades</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* 6. WEBHOOKS EXTERNOS */}
            {editRule?.tipo === "run_webhook" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="code" size={14} /> Enviar datos a Webhook</div>
                <Campo label="URL del Endpoint (Make/Zapier)">
                  <Inp value={editRule.config?.url} onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, url: e.target.value } })} placeholder="https://..." />
                </Campo>
              </div>
            )}

            {/* 7. GESTIÓN DE ETIQUETAS */}
            {(editRule?.tipo === "add_tag" || editRule?.tipo === "remove_tag") && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="tag" size={14} /> Gestión de Etiquetas</div>
                <Campo label="Etiqueta a Gestionar">
                  <Inp value={editRule.config?.tag} onChange={e => setEditRule({ ...editRule, config: { ...editRule.config, tag: e.target.value } })} placeholder="Ej: VIP, Prioritario, Prospecto..." />
                </Campo>
              </div>
            )}

            {/* 8. CLIMATIZACIÓN DE DATOS */}
            {editRule?.tipo === "data_clean" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: T.white, display: "flex", alignItems: "center", gap: 8 }}><Ico k="broom" size={14} /> Limpieza de Datos</div>
                <ThemedSelect
                  label="Campo a limpiar:"
                  value={editRule.config?.campo || ""}
                  options={crmFields.map(f => ({ id: f.id, label: f.label }))}
                  onChange={v => setEditRule({ ...editRule, config: { ...editRule.config, campo: v } })}
                />
                <ThemedSelect
                  label="Método de limpieza:"
                  value={editRule.config?.metodo || "upper"}
                  options={[{ id: "upper", label: "TODO MAYÚSCULAS" }, { id: "lower", label: "todo minúsculas" }, { id: "trim", label: "Quitar espacios extra" }]}
                  onChange={v => setEditRule({ ...editRule, config: { ...editRule.config, metodo: v } })}
                />
              </div>
            )}
