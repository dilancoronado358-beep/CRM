...
export const Contactos = ({ db, setDb, guardarEnSupa, eliminarDeSupa, focusContactoId, setFocusContactoId }) => {
  ...
  // Soporte para Navegación Profunda (Deep Linking)
  useEffect(() => {
    if (focusContactoId && db.contactos?.length > 0) {
      const c = db.contactos.find(x => x.id === focusContactoId);
      if (c) {
        setDetalle(c);
        setFocusContactoId(null);
      }
    }
  }, [focusContactoId, db.contactos]);
  ...
  const eliminarSeleccionados = async () => {
    if (!confirm(`¿Estás seguro de eliminar los ${seleccionados.size} contactos seleccionados de forma permanente?`)) return;
    const arrayIds = Array.from(seleccionados);
    setDb(d => ({ ...d, contactos: d.contactos.filter(c => !seleccionados.has(c.id)) }));
    setSeleccionados(new Set());
    
    // Eliminación asíncrona múltiple de Supabase
    Promise.all(arrayIds.map(idStr => eliminarDeSupa("contactos", idStr))).then(() => {
      sileo.success(`${arrayIds.length} contactos eliminados correctamente.`);
    }).catch(e => {
      console.error(e);
      sileo.error("Ocurrió un error al eliminar algunos contactos de la base de datos.");
    });
  };
        ...
        actions={
          <div style={{ display: "flex", gap: 12 }}>
            {seleccionados.size > 0 && (
              <Btn variant="peligro" onClick={eliminarSeleccionados}>
                <Ico k="trash" size={14} /> Eliminar ({seleccionados.size})
              </Btn>
            )}
            <MenuDatos onImport={() => {
              if(!checkPlanLimit(db, "contactos")) return sileo.error(getPlanLimitError("contactos"));
              setShowImport(true);
            }} onExport={handleExport} />
            <Btn onClick={() => { 
              if(!checkPlanLimit(db, "contactos")) return sileo.error(getPlanLimitError("contactos"));
              setEditando(null); setShowForm(true); 
            }}><Ico k="plus" size={14} />Nuevo Contacto</Btn>
          </div>
        } />
      ...
      {vista === "tabla" && (
        <Tarjeta>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <CabeceraTabla cols={[
              <input type="checkbox" onChange={handleSelectAll} checked={filtrados.length > 0 && seleccionados.size === filtrados.length} style={{ width: 16, height: 16, cursor: "pointer", accentColor: T.teal }} />,
              "Contacto", "Empresa", "Estado", "Score", "Valor", "Fuente", "Últ. Contacto", ""
            ]} />
            <tbody>
              {filtrados.map(c => {
                ...
                return (
                  <FilaTabla key={c.id} onClick={() => setDetalle(c)}>
                    <Celda style={{ width: 40 }} onClick={e => e.stopPropagation()}>
                      <input type="checkbox" checked={seleccionados.has(c.id)} onChange={e => handleSelect(c.id, e.target.checked)} style={{ width: 16, height: 16, cursor: "pointer", accentColor: T.teal }} />
                    </Celda>
              ...
              {filtrados.length === 0 && <tr><td colSpan={9}><Vacio text="No se encontraron contactos" /></td></tr>}
            </tbody>
          </table>
        </Tarjeta>
      )}
      ...
      {vista === "tarjetas" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 16 }}>
          {filtrados.map(c => {
            const ec = ESTADO_CFG[c.estado] || ESTADO_CFG.lead;
            return (
              <Tarjeta key={c.id} onClick={() => setDetalle(c)} style={{ padding: 20, cursor: "pointer", transition: "all .2s", display: "flex", flexDirection: "column", position: "relative", border: seleccionados.has(c.id) ? `1px solid ${T.teal}` : undefined }}
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor = seleccionados.has(c.id) ? T.teal : T.border; e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "0 1px 3px rgba(0,0,0,0.05)"; }}>
                
                <div style={{ position: "absolute", top: 16, right: 16, zIndex: 10 }} onClick={e => e.stopPropagation()}>
                  <input type="checkbox" checked={seleccionados.has(c.id)} onChange={e => handleSelect(c.id, e.target.checked)} style={{ width: 18, height: 18, cursor: "pointer", accentColor: T.teal }} />
                </div>

                <div style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "flex-start", paddingRight: 24 }}>
