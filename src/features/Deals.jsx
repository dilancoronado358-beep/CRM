...
export const Deals = ({ db, setDb, guardarEnSupa, eliminarDeSupa, setModulo, setFocusDealId }) => {
  ...
  return (
    <div>
      <EncabezadoSeccion title="Deals y Oportunidades" sub={`${filtrados.length} oportunidades filtradas Â· ${money(total)} generados en pipeline`} />
      
      <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap", maxWidth: 900 }}>
        <KPI label="Valor del Pipeline" value={money(total)} color={T.teal} icon="funnel" />
        ...
        <KPI label="Oportunidades Pendientes" value={filtrados.filter(d => !esGanado(d)).length} color={T.amber} icon="clock" />
      </div>

      <Tarjeta style={{ padding: 20, marginBottom: 20, zIndex: 10 }}>
      ...
      <Tarjeta>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <CabeceraTabla cols={["Deal", "Contacto", "Pipeline Â· Etapa", "Valor", "Probabilidad", "Cierre", "Responsable", ""]} />
          <tbody>
            {filtrados.map(deal => {
              ...
              return (
                <FilaTabla key={deal.id} onClick={() => {
                  ...
                  <Celda>{contacto ? <div style={{ display: "flex", gap: 10, alignItems: "center" }}><Av text={contacto.avatar} color={contacto.color} size={28} fs={10} /><span style={{ fontSize: 13, fontWeight: 600 }}>{contacto.nombre}</span></div> : <span style={{ color: T.whiteFade }}>â</span>}</Celda>
