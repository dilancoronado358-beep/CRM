      ...
      srcDoc={`
        ...
        <html>
          <head>
            ...
            <style>
              body { font-family: 'Inter', -apple-system, sans-serif; color: #1e293b; margin: 0; padding: 16px; overflow-x: hidden; line-height: 1.6; background: #fff; font-size: 14px; zoom: 0.75; }
              .email-wrapper { max-width: 650px; margin: 0 auto; }
              img { max-width: 100% !important; height: auto !important; }
              table { max-width: 100% !important; }
              td, div, p, section { max-width: 100% !important; box-sizing: border-box !important; }
              a { color: #06B6D4; text-decoration: none; }
              * { word-wrap: break-word; }
            </style>
          </head>
          <body><div class="email-wrapper">${content}</div></body>
        </html>
      `}
      style={{ width: "100%", height: "100%", border: "none", background: "#FFFFFF", borderRadius: 16, overflow: "auto", minHeight: 0 }}
    />
  );
});
...
export const ModuloEmail = ({ db, setDb, guardarEnSupa, eliminarDeSupa, t, cargandoFondo, setModulo, focusEmailId, setFocusEmailId }) => {
  ...
  useEffect(() => {
    if (db.email_accounts?.length > 0) {
      const stillExists = db.email_accounts.find(a => a.id === selectedAccountId);
      if (!stillExists) setSelectedAccountId(db.email_accounts[0].id);
    } else {
  ...
  const msgs = selectedAccountId
    ? db.emails
        .filter(e => e.carpeta === carpeta && e.account_id === selectedAccountId)
  ...
  const handleFileChange = async (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const { data, error } = await sb.storage.from('email-attachments').upload(safeName, file, { 
          upsert: true,
          contentType: file.type || 'application/octet-stream'
        });
        if (error) {
          // Si el bucket no existe, intentar crear y reintentar
          if (error.message?.includes('not found') || error.statusCode === 404 || error.message?.includes('Bucket')) {
            console.error('Bucket email-attachments no existe. Creando...');
            await sb.storage.createBucket('email-attachments', { public: true, allowedMimeTypes: null, fileSizeLimit: 52428800 });
            const { data: d2, error: e2 } = await sb.storage.from('email-attachments').upload(safeName, file, { upsert: true, contentType: file.type || 'application/octet-stream' });
            if (e2) throw e2;
          } else {
            throw error;
          }
        ...
        toast.error(`Error Subiendo ${file.name}: ${err.message || err.error || 'Error desconocido'}`); 
      }
    }
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 110px)", background: T.bg0, borderRadius: 32, overflow: "hidden", border: `1.5px solid ${T.whiteFade}10`, boxShadow: "0 30px 80px rgba(0,0,0,0.15)", position: "relative" }}>
      ...
      {/* 2. LISTA DE MENSAJES */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: "none", transition: "all 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}>
      ...
      {/* 3. LECTOR DE MENSAJES (MODAL) */}
      <Modal open={!!emailFocus} onClose={() => setEmailFocus(null)} title="" width="70vw">
        {emailFocus && (
          <div style={{ display: "flex", flexDirection: "column", height: "90vh", background: "transparent" }}>
            <div style={{ padding: "0 24px 16px", borderBottom: `1px solid ${T.whiteFade}05`, background: "transparent" }}>
          ...
          <div style={{ flex: 1, padding: "0 24px 12px", minHeight: 0, overflow: "hidden" }}>
            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 20, padding: 6, border: `1px solid ${T.whiteFade}08`, height: "100%" }}>
        ...
        <div style={{ padding: "20px 24px", borderTop: `1px solid ${T.whiteFade}08`, background: "rgba(255,255,255,0.01)" }}>
          <div 
            onClick={() => {
              setF({ 
                para: emailFocus.de.includes('<') ? emailFocus.de.match(/<([^>]+)>/)?.[1] || emailFocus.de : emailFocus.de, 
                cc: "", 
                bcc: "", 
                asunto: (emailFocus.asunto || "").startsWith("Re:") ? emailFocus.asunto : "Re: " + (emailFocus.asunto || "(Sin asunto)"), 
                cuerpo: "",
                tipo: "texto",
                plantillaId: ""
              });
            ...
            style={{ background: "rgba(0,0,0,0.2)", border: `1px solid ${T.whiteFade}10`, borderRadius: 16, padding: "16px 24px", cursor: "text", color: T.whiteDim, fontSize: 14, display: "flex", alignItems: "center", gap: 12, transition: "all 0.2s" }}
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(0,0,0,0.3)"; e.currentTarget.style.border = `1px solid ${T.whiteFade}20`; }}
            onMouseLeave={e => { e.currentTarget.style.background = "rgba(0,0,0,0.2)"; e.currentTarget.style.border = `1px solid ${T.whiteFade}10`; }}
          >
            <Ico k="reply" size={18} style={{ color: T.teal }} />
            <span>Haz clic aquí para responder usando el <strong>Editor Completo (Plantillas, IA, Adjuntos)...</strong></span>
          </div>
        </div>
        </div>
        )}
      ...
      <Modal open={showRedactar} onClose={() => setShowRedactar(false)} title="Nuevo Mensaje" width="95vw">
             ...
             <div style={{ position: "relative" }}>
               {f.tipo === "html" ? (
                 <textarea value={f.cuerpo} onChange={s("cuerpo")} 
                   style={{ width: "100%", height: "60vh", background: "rgba(0,0,0,0.2)", border: "none", color: T.whiteOff, padding: 20, fontSize: 13, outline: "none", resize: "none", lineHeight: 1.6, fontFamily: "monospace" }} 
                   placeholder="Código HTML del mensaje..." 
                 />
               ) : (
                 <div style={{ width: "100%", height: "60vh", background: "#fff", overflowY: "auto" }}>
