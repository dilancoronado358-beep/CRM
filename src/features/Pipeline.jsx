...
const FormDeal = ({ db, setDb, f, setF, editDeal, onGuardar, onCancelar, guardarEnSupa, ejecutarAutomaciones, setModulo, setShowConfigCampos, focusEmailId, setFocusEmailId, focusDealId, setFocusDealId, setFocusContactoId }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24, padding: 24, background: T.bg2, color: T.white }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 32, height: "100%" }}>
        
        {/* COLUMNA IZQUIERDA: FULL INFO (SCROLLABLE AREA) */}
        <div className="custom-scrollbar" style={{ display: "flex", flexDirection: "column", gap: 24, minHeight: "78vh", maxHeight: "82vh", overflowY: "auto", paddingRight: 12 }}>
          ...
          <div style={{ 
            ...
            boxShadow: "0 10px 40px rgba(0,0,0,0.03)"
             ...
             <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
                ...
                <div style={{ background: T.bg3, padding: "8px 16px", borderRadius: 12, fontSize: 11, fontWeight: 900, color: T.whiteDim, border: `1px solid ${T.borderHi}` }}>AI SCORE: {calculateLeadScore(db, f)}</div>
             </div>
             ...
             <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
               <Campo label="TÍTULO DEL NEGOCIO">
                 ...
                 }} style={{ fontSize: 26, fontWeight: 900, background: "transparent", border: "none", borderBottom: `2px solid ${T.borderHi}`, borderRadius: 0, height: 50, padding: "0 0 8px 0", color: T.white }} placeholder="Ej: Proyecto Expansión Q2" />
               </Campo>
                 ...
                 <Campo label="CONFIANZA DEL CIERRE">
                   <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                     <LocalInput type="number" value={f.prob} onCommit={v => {
                       ...
                       if (editDeal) guardarEnSupa("deals", { ...editDeal, ...nf });
                     }} style={{ fontWeight: 900, fontSize: 32, background: "transparent", border: "none", padding: 0, textAlign: "right", width: "100%", color: T.whiteDim }} />
                     <span style={{ fontSize: 20, fontWeight: 900, color: T.whiteFade }}>%</span>
                   </div>
                 </Campo>
               </div>
               <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, paddingTop: 16, borderTop: `1px solid ${T.border}` }}>
                  <Campo label="FECHA CREACIÓN">
                    <LocalInput type="date" value={f.creado || ""} onCommit={v => {
                      const nf = { ...f, creado: v }; setF(nf);
                      if (editDeal) guardarEnSupa("deals", { ...editDeal, ...nf });
                    }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 48, padding: "0 16px", fontSize: 13, fontWeight: 700, color: T.white }} />
                  </Campo>
                  <Campo label="FECHA ESTIMADA CIERRE"><LocalInput type="date" value={f.fecha_cierre || ""} onCommit={v => {
                    const nf = { ...f, fecha_cierre: v }; setF(nf);
                    if (editDeal) guardarEnSupa("deals", { ...editDeal, ...nf });
                  }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 48, padding: "0 16px", fontSize: 13, fontWeight: 700, color: T.white }} /></Campo>
                 <Campo label="ETIQUETAS"><LocalInput value={Array.isArray(f.etiquetas) ? f.etiquetas.join(", ") : (f.etiquetas || "")} onCommit={v => {
                   ...
                   if (editDeal) guardarEnSupa("deals", { ...editDeal, ...nf });
                 }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 48, padding: "0 16px", fontSize: 13, fontWeight: 700, color: T.white }} placeholder="vip, prospecto, urgente" /></Campo>
                 
                 {customFieldsDef.filter(cf => cf.seccion === "detalles").sort((a,b) => (a.orden || 0) - (b.orden || 0)).map(cf => {
                  ...
                  return (
                    <Campo key={cf.id} label={cf.nombre.toUpperCase()}>
                      {cf.tipo === "lista" ? (
                        <Sel value={val} onChange={e => handleChange(e.target.value)} onBlur={handleBlur} multiple={true} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 50, color: T.white, fontWeight: 600 }}>
                          <option value="">— Seleccionar —</option>
                          {cf.opciones?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                        </Sel>
                      ) : (
                        <LocalInput value={val} onCommit={v => { handleChange(v); handleBlur(); }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, color: T.white, height: 50, fontSize: 14, fontWeight: 700, padding: "0 18px" }} />
                      )}
                    </Campo>
                  );
                })}
           ...
           <div style={{ 
             ...
             <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
                <div style={{ width: 32, height: 32, borderRadius: 12, background: "#0EA5E915", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Ico k="users" size={14} style={{ color: T.teal }} />
                </div>
                <div style={{ fontSize: 13, fontWeight: 900, color: T.white, letterSpacing: ".05em", textTransform: "uppercase" }}>Relaciones & Staff</div>
             </div>
             ...
             <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
               <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
                  <Campo label="CONTACTO PRINCIPAL">
                    ...
                    }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 52, color: T.white, fontWeight: 600 }}>
               ...
               {selContacto && (
                 <div style={{ background: "rgba(14, 165, 233, 0.04)", padding: 24, borderRadius: 24, border: "1px solid rgba(14, 165, 233, 0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 14, color: T.whiteDim, fontWeight: 700 }}>
                        <Ico k="mail" size={14} style={{ color: T.teal }} /> {selContacto.email || "Sin email"}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 14, color: T.whiteDim, fontWeight: 700 }}>
                        <Ico k="phone" size={14} style={{ color: "#10B981" }} /> {selContacto.telefono || "Sin teléfono"}
                      </div>
                    </div>
                    <Btn variant="turquesa" size="sm" onClick={() => {
                        setFocusContactoId(selContacto.id);
                        setModulo("contactos");
                        onCancelar();
                    }} style={{ borderRadius: 14, padding: "10px 20px", fontWeight: 800 }}>Ver Perfil</Btn>
                 </div>
               )}
                ...
                <Campo label="RESPONSABLE ASIGNADO">
                  <Sel value={f.responsable} onChange={async e => {
                    ...
                    {(db.usuariosApp || []).map(u => {
                      ...
                      return <option key={u.id} value={u.name}>{u.name} {org ? `(${org.nombre})` : ""}</option>
                    })}
                  </Sel>
                </Campo>

               {customFieldsDef.filter(cf => cf.seccion === "relaciones").sort((a,b) => (a.orden || 0) - (b.orden || 0)).map(cf => {
                  const val = f.custom_fields?.[cf.id] || "";
                  const handleChange = (v) => setF(prev => ({ ...prev, custom_fields: { ...(prev.custom_fields || {}), [cf.id]: v } }));
                  const handleBlur = async () => { if (editDeal) await guardarEnSupa("deals", { ...editDeal, custom_fields: f.custom_fields }); };
                  return (
                    <Campo key={cf.id} label={cf.nombre.toUpperCase()}>
                      {cf.tipo === "lista" ? (
                        <Sel value={val} onChange={e => handleChange(e.target.value)} onBlur={handleBlur} multiple={true} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, height: 52, color: T.white, fontWeight: 600 }}>
                          <option value="">— Seleccionar —</option>
                          {cf.opciones?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                        </Sel>
                      ) : (
                        <LocalInput value={val} onCommit={v => { handleChange(v); handleBlur(); }} style={{ background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 16, color: T.white, height: 52, fontSize: 14, fontWeight: 700, padding: "0 18px" }} />
                      )}
                    </Campo>
                  );
                })}
             ...
             boxShadow: "0 10px 40px rgba(0,0,0,0.05)"
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                 <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 900, color: T.white, letterSpacing: ".05em", textTransform: "uppercase" }}>Atributos Avanzados</div>
                 </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
                 {customFieldsDef.filter(cf => (cf.seccion || "avanzado") === "avanzado").sort((a,b) => (a.orden || 0) - (b.orden || 0)).map(cf => {
                   const val = f.custom_fields?.[cf.id] || "";
                   const handleChange = (v) => setF(prev => ({ ...prev, custom_fields: { ...(prev.custom_fields || {}), [cf.id]: v } }));
                   const handleBlur = async () => { if (editDeal) await guardarEnSupa("deals", { ...editDeal, custom_fields: f.custom_fields }); };
                 ...
                 {customFieldsDef.filter(cf => (cf.seccion || "avanzado") === "avanzado").length === 0 && (
                   <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "40px", border: "2px dashed #E2E8F0", borderRadius: 24, background: T.bg2 }}>
                     <div style={{ color: T.whiteDim, fontSize: 14, fontWeight: 700, marginBottom: 16 }}>Personaliza tu flujo con campos únicos.</div>
           ...
           <div style={{ 
             ...
             boxShadow: "0 10px 40px rgba(0,0,0,0.05)"
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
                 ...
                 <div style={{ fontSize: 13, fontWeight: 900, color: T.white, letterSpacing: ".05em", textTransform: "uppercase" }}>Notas Estratégicas</div>
              </div>
              <textarea 
                 value={f.notas || ""} 
                 onChange={e => setF(prev => ({ ...prev, notas: e.target.value }))}
                 onBlur={async e => { e.target.style.borderColor = ""; e.target.style.boxShadow = "none"; if (editDeal) await guardarEnSupa("deals", { ...editDeal, notas: f.notas }); }}
                 placeholder="Describa aquí el plan de acción, puntos de dolor detectados o próximos pasos críticos del negocio..."
                 style={{ width: "100%", minHeight: 180, background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 24, padding: 24, color: T.white, fontSize: 15, fontFamily: "inherit", resize: "none", outline: "none", lineHeight: 1.8, transition: "all .4s", fontWeight: 500 }}
                 onFocus={e => { e.target.style.borderColor = T.teal; e.target.style.boxShadow = "0 0 0 3px rgba(20, 184, 166, 0.15)"; }}
              />
           </div>
           ...
           <div style={{ 
             ...
             <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
                 ...
                 <div style={{ fontSize: 13, fontWeight: 900, color: T.white, letterSpacing: ".05em", textTransform: "uppercase" }}>Vault de Documentos</div>
             </div>
             ...
             <div 
               ...
               <div style={{ textAlign: "center" }}>
                 <div style={{ fontSize: 17, color: T.white, fontWeight: 900, marginBottom: 8 }}>Subir Documentación</div>
                 <div style={{ fontSize: 13, color: T.whiteDim, fontWeight: 700 }}>Click para explorar o arrastra tus archivos aquí</div>
               </div>
             </div>
             ...
             {f.archivos?.length > 0 && (
               <div style={{ marginTop: 32, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                 {(f.archivos || []).map(a => (
                   <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: T.bg0, padding: "16px 20px", borderRadius: 24, border: `1px solid ${T.borderHi}`, boxShadow: "0 4px 15px rgba(0,0,0,0.02)" }}>
                     <div style={{ display: "flex", alignItems: "center", gap: 14, overflow: "hidden" }}>
                       ...
                       <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
                         <span style={{ fontSize: 13, color: T.white, fontWeight: 800, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>{a.nombre}</span>
                         <span style={{ fontSize: 10, color: T.whiteFade, fontWeight: 800 }}>{a.size}</span>
                       </div>
                     </div>
        ...
        <div style={{ minHeight: "78vh", background: T.bg1, borderRadius: 32, border: `1px solid ${T.white}05`, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          
          <div style={{ display: "flex", background: T.bg2, padding: 4, gap: 4 }}>
            ...
            {rightTab === "finanzas" && (
              <div style={{ padding: 40 }}>
                 ...
                 <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                   <div style={{ padding: 32, background: T.bg2, borderRadius: 24, border: `1px solid ${T.white}08` }}>
            ...
            {rightTab === "historial" && (
              <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 12 }}>
                {(db.auditoria || []).filter(a => a.entidad_id === editDeal?.id).sort((a,b) => new Date(b.creado) - new Date(a.creado)).map(a => (
                  <div key={a.id} style={{ display: "flex", gap: 16, padding: "16px 20px", background: T.bg2, borderRadius: 16, border: `1px solid ${T.white}05` }}>
...
export const Pipeline = ({ db, setDb, guardarEnSupa, eliminarDeSupa, t, setModulo, focusEmailId, setFocusEmailId, focusDealId, setFocusDealId, focusContactoId, setFocusContactoId }) => {
  ...
  const [nuevoCampo, setNuevoCampo] = useState({ nombre: "", tipo: "cadena", opciones: "", seccion: "avanzado", orden: 0 });
  const [editandoCampo, setEditandoCampo] = useState(null); // ID del campo siendo editado
  ...
  // ?? DEEP NAVIGATION: Abrir negocio enfocado desde otros módulos
  useEffect(() => {
    if (focusDealId && db.deals?.length > 0) {
      const target = db.deals.find(d => d.id === focusDealId);
      if (target) {
        setPlActivo(target.pipeline_id);
        setEditDeal(target);
        setShowDealForm(true);
        setFocusDealId(null);
      }
    }
  }, [focusDealId, db.deals]);
  ...
  const crearCampo = async () => {
    ...
    const campo = {
      id: editandoCampo || ("cf" + uid()),
      nombre: nuevoCampo.nombre,
      tipo: nuevoCampo.tipo,
      opciones: nuevoCampo.tipo === "lista" ? (typeof nuevoCampo.opciones === 'string' ? nuevoCampo.opciones.split(",").map(o => o.trim()).filter(Boolean) : nuevoCampo.opciones) : [],
      entidad: "deal",
      seccion: nuevoCampo.seccion || "avanzado",
      orden: editandoCampo ? nuevoCampo.orden : (db.campos_personalizados?.length || 0)
    };

    // Sincronizar con Supabase y estado local
    setDb(d => {
       const listaActual = d.campos_personalizados || [];
       const existe = listaActual.find(c => c.id === campo.id);
       const nuevaLista = existe 
         ? listaActual.map(c => c.id === campo.id ? campo : c) 
         : [...listaActual, campo];
       return { ...d, campos_personalizados: nuevaLista };
    });
    
    await guardarEnSupa("campos_personalizados", campo);

    setNuevoCampo({ nombre: "", tipo: "cadena", opciones: "", seccion: "avanzado", orden: 0 });
    setEditandoCampo(null);
    setShowConfigCampos(false); 
  };

  const moverCampo = async (id, dir) => {
    const list = [...(db.campos_personalizados || [])].sort((a, b) => (a.orden || 0) - (b.orden || 0));
    ...
    setDb(d => ({ ...d, campos_personalizados: newList }));
    await guardarEnSupa("campos_personalizados", a);
    await guardarEnSupa("campos_personalizados", b);
  };

  const prepararEdicionCampo = (cf) => {
    setEditandoCampo(cf.id);
    setNuevoCampo({ 
      nombre: cf.nombre, 
      tipo: cf.tipo, 
      opciones: Array.isArray(cf.opciones) ? cf.opciones : [],
      seccion: cf.seccion || "avanzado",
      orden: cf.orden || 0
    });
  };
    ...
    <div>
      <EncabezadoSeccion title="Pipeline CRM" sub="Gestiona tus oportunidades en etapas visuales"
        actions={
           <div style={{ display: "flex", gap: 12 }}>
            <MenuVistas value={tab} onChange={setTab} />
            <Btn variant="secundario" onClick={() => setShowConfigCampos(true)} title="Configurar Campos Globales"><Ico k="cog" size={14} /> Campos</Btn>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", background: T.bg1, border: `1px solid ${T.borderHi}`, borderRadius: 10, padding: "4px 12px", gap: 10, flex: 1, minWidth: 250 }}>
          ...
          <Btn variant="fantasma" size="sm" onClick={() => {
              if(!checkPlanLimit(db, "pipelines")) return sileo.error(getPlanLimitError("pipelines"));
              setShowNuevoPL(true);
            }}><Ico k="plus" size={14} />Nuevo Pipeline</Btn>
      </div>
        ...
        <div style={{ background: T.bg1, border: `1px solid ${T.borderHi}`, borderRadius: 12, padding: 16, marginBottom: 20, display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-end", animation: "slide-down 0.2s ease-out" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <select value={fResp} onChange={e => setFResp(e.target.value)} style={{ background: T.bg2, color: T.white, border: `1px solid ${T.borderHi}`, borderRadius: 8, padding: "6px 12px", fontSize: 13, cursor: "pointer", outline: "none" }}>
              ...
              {(db.usuariosApp || []).filter(u => !u.org_id || u.org_id === db.usuario?.org_id).map(u => <option key={u.id} value={u.name}>{u.name}</option>)}
            </select>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {tab === "kanban" && pipeline && (
        <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 20, minHeight: "70vh", alignItems: "flex-start" }}>
          {pipeline.etapas?.map((etapa, etapaIdx) => {
            // La primera columna también captura leads con etapa_id inválida (huérfanos)
            const etapaIdsValidas = new Set((pipeline.etapas || []).map(e => e.id));
            const etDeals = plDeals.filter(d => {
              if (d.etapa_id === etapa.id) return true;
              // Solo en la primera columna: incluir leads sin etapa válida
              if (etapaIdx === 0 && !etapaIdsValidas.has(d.etapa_id)) return true;
              return false;
            });
            const isOver = dragSobre === etapa.id;
            const totalEtapa = etDeals.reduce((s, d) => s + (d.valor || 0), 0);
            const etapaColor = etapa.color || T.whiteDim;
            const colBg = isOver ? (etapaColor.startsWith("#") ? etapaColor + "08" : "rgba(255,255,255,0.03)") : T.bg2;
            return (
              <div key={etapa.id}
                style={{ 
                  minWidth: 220, maxWidth: 220, display: "flex", flexDirection: "column", flexShrink: 0, 
                  borderRadius: 14, 
                  background: colBg, 
                  border: `1px solid ${isOver ? etapaColor : T.borderHi}`, 
                ...
                <div style={{ background: etapaColor, padding: "7px 10px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
                ...
                <button onClick={() => { setEditDeal(null); setPreEtapa(etapa.id); setShowDealForm(true); }}
                  style={{ background: "transparent", border: "none", borderBottom: `1px solid ${T.borderHi}`, padding: "9px 14px", color: etapaColor, cursor: "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5, fontFamily: "inherit", textAlign: "left", width: "100%", transition: "background .15s" }}
                  onMouseEnter={e => e.currentTarget.style.background = etapaColor.startsWith("#") ? etapaColor + "12" : etapaColor}
      ...
      {tab === "lista" && pipeline && (
        <div style={{ background: T.bg1, border: `1px solid ${T.borderHi}`, borderRadius: 16, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: T.bg2, borderBottom: `1px solid ${T.borderHi}` }}>
            ...
            <tbody>
              {plDeals.map(deal => {
                ...
                const etapaColorSafe = etapa.color || T.whiteDim;
                const contacto = db.contactos?.find(c => c.id === deal.contacto_id);
                const dStat = getDealStatus(deal);
                const isSelected = selectedIds.includes(deal.id);
                ...
                return (
                  <tr key={deal.id}
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "inline-block", background: etapaColorSafe.startsWith("#") ? etapaColorSafe + "20" : etapaColorSafe, color: etapaColorSafe, border: `1px solid ${etapaColorSafe}`, padding: "2px 8px", borderRadius: 12, fontSize: 10, fontWeight: 800, textTransform: "uppercase" }}>
                        {etapa.nombre || "Sin etapa"}
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 20, height: 20, borderRadius: "50%", background: etapaColorSafe, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 900 }}>
      ...
      <SidePanel open={showDealForm} onClose={() => { setShowDealForm(false); setEditDeal(null); }} title={editDeal ? "Editar Deal" : "Nuevo Deal"} width="80%">
        <FormDeal
          db={db}
          setDb={setDb}
          f={f}
          ...
          setFocusDealId={setFocusDealId}
          setFocusContactoId={setFocusContactoId}
        />
      </SidePanel>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ background: T.bg2, padding: 16, borderRadius: 10, border: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h4 style={{ margin: 0, fontSize: 13, color: T.white }}>{editandoCampo ? "Editar Campo" : "Crear Nuevo Campo"}</h4>
              {editandoCampo && <Btn variant="fantasma" size="xs" onClick={() => { setEditandoCampo(null); setNuevoCampo({ nombre: "", tipo: "cadena", opciones: "", seccion: "avanzado", orden: 0 }); }} style={{ color: T.red }}>Cancelar Edición</Btn>}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 12 }}>
                ...
                <Campo label="Sección">
                  <Sel value={nuevoCampo.seccion || "avanzado"} onChange={e => setNuevoCampo(p => ({ ...p, seccion: e.target.value }))}>
              ...
              {nuevoCampo.tipo === "lista" && (
                <Campo label="Opciones de la Lista">
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <Inp 
                        ...
                        onKeyDown={e => {
                          if (e.key === "Enter") {
                            ...
                            if (val) {
                              const currentOptions = Array.isArray(nuevoCampo.opciones) ? nuevoCampo.opciones : (nuevoCampo.opciones ? nuevoCampo.opciones.split(",").map(o => o.trim()).filter(Boolean) : []);
                              if (!currentOptions.includes(val)) {
                                setNuevoCampo(p => ({ ...p, opciones: [...currentOptions, val] }));
                              }
                              e.target.value = "";
                            }
                          }
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, minHeight: 40, padding: "8px 12px", background: "rgba(255,255,255,0.02)", border: `1px dashed ${T.borderHi}`, borderRadius: 12 }}>
                      {(Array.isArray(nuevoCampo.opciones) ? nuevoCampo.opciones : (nuevoCampo.opciones ? nuevoCampo.opciones.split(",").map(o => o.trim()).filter(Boolean) : [])).map((opt, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, background: T.tealSoft, color: T.teal, padding: "4px 10px", borderRadius: 8, fontSize: 11, fontWeight: 800, border: `1px solid ${T.teal}30` }}>
                          {opt}
                          <div onClick={() => {
                            const currentOptions = Array.isArray(nuevoCampo.opciones) ? nuevoCampo.opciones : (nuevoCampo.opciones ? nuevoCampo.opciones.split(",").map(o => o.trim()).filter(Boolean) : []);
                            setNuevoCampo(p => ({ ...p, opciones: currentOptions.filter(o => o !== opt) }));
                          }} style={{ cursor: "pointer", display: "flex", opacity: 0.6 }} onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => e.currentTarget.style.opacity = 0.6}>
                            <Ico k="x" size={10} />
                          </div>
                        </div>
                      ))}
                      {(!nuevoCampo.opciones || (Array.isArray(nuevoCampo.opciones) && nuevoCampo.opciones.length === 0)) && (
                        <span style={{ fontSize: 11, color: T.whiteDim, opacity: 0.5, fontStyle: "italic" }}>Sin opciones añadidas...</span>
                      )}
                    </div>
                  </div>
                </Campo>
              )}
              <Btn onClick={crearCampo} disabled={!nuevoCampo.nombre.trim()} style={{ marginTop: 8 }}>{editandoCampo ? "Guardar Cambios" : "Crear campo global"}</Btn>
            </div>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <h4 style={{ margin: 0, fontSize: 13, color: T.whiteDim }}>Campos Existentes</h4>
            {[...(db.campos_personalizados || [])].sort((a,b) => (a.orden || 0) - (b.orden || 0)).map((cf, idx, arr) => (
              <div key={cf.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: T.bg1, padding: "10px 14px", borderRadius: 8, border: `1px solid ${T.borderHi}` }}>
                <div>
                  ...
                  <div style={{ fontSize: 10, color: T.teal, fontWeight: 800, textTransform: "uppercase" }}>{cf.seccion || "avanzado"} • {cf.tipo}</div>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    <button disabled={idx === 0} onClick={() => moverCampo(cf.id, -1)} style={{ background: "none", border: "none", color: idx === 0 ? T.whiteFade : T.teal, cursor: "pointer", padding: 0 }}><Ico k="chevron-down" size={10} style={{ transform: "rotate(180deg)" }} /></button>
                    <button disabled={idx === arr.length - 1} onClick={() => moverCampo(cf.id, 1)} style={{ background: "none", border: "none", color: idx === arr.length - 1 ? T.whiteFade : T.teal, cursor: "pointer", padding: 0 }}><Ico k="chevron-down" size={10} /></button>
                  </div>
                  <Btn variant="fantasma" size="sm" onClick={() => prepararEdicionCampo(cf)} style={{ color: T.whiteDim }}><Ico k="edit" size={12} /></Btn>
      ...
      {selectedIds.length > 0 && (
        <div style={{ 
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <Btn variant="fantasma" size="xs" onClick={() => setSelectedIds([])} style={{ color: T.whiteDim, fontWeight: 700 }}>Deseleccionar</Btn>
          </div>

          <div style={{ height: 32, width: 1, background: "rgba(0,0,0,0.05)" }} />

          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <span style={{ fontSize: 11, color: T.whiteFade, fontWeight: 800, letterSpacing: ".05em" }}>ACCIONES:</span>

            <Sel value="" onChange={e => handleBulkStage(e.target.value)} style={{ width: 160, fontSize: 12, height: 40, background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 12, color: T.white }}>
              <option value="">Cambiar Etapa...</option>
              {pipeline?.etapas?.map(et => <option key={et.id} value={et.id} style={{ color: T.white }}>{et.nombre}</option>)}
            </Sel>

            <Sel value="" onChange={e => handleBulkResp(e.target.value)} style={{ width: 160, fontSize: 12, height: 40, background: T.bg2, border: `1px solid ${T.borderHi}`, borderRadius: 12, color: T.white }}>
              <option value="">Cambiar Responsable...</option>
              {(db.usuariosApp || []).filter(u => !u.org_id || u.org_id === db.usuario?.org_id).map(u => <option key={u.id} value={u.name} style={{ color: T.white }}>{u.name}</option>)}
            </Sel>
