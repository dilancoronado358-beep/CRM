...
const TABLAS_CRITICAS = [
  "organizacion", "pipelines", "contactos", "deals", "tareas", "empresaConfigs", "usuariosApp", "empresas", "whatsapp_accounts", "productos", "facturas"
];

// Tablas que pueden cargarse en segundo plano (histórico/pesadas)
const TABLAS_FONDO = [
  "actividades", "emails", "email_accounts", "notas",
...
export function useSupaState() {
      ...
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload) => {
        ...
        setDbRaw(prev => {
          ...
          if (eventType === 'INSERT' || eventType === 'UPDATE') {
            ...
            // ACTUALIZACIÓN CRÍTICA: Si el registro que cambió es el del usuario actual, 
            // actualizamos también el objeto db.usuario para que los permisos se reflejen en vivo.
            let userUpdate = {};
            if (record.id === prev.usuario?.id) {
              userUpdate = { usuario: { ...prev.usuario, ...record } };
  ...
  const guardarEnSupa = async (tabla, registro) => {
    try {
      ...
      if (tabla !== "organizacion" && (!payload.org_id || !esUUID(payload.org_id))) {
        const uOrg = db.usuario?.org_id;
        payload.org_id = esUUID(uOrg) ? uOrg : orgIdDefecto;
      }
